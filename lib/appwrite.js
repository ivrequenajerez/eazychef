...
/* Platos - Eliminar */
export const deleteVideo = async (title, form) => {
  try {
    // Obtener el documentId basado en el título
    const documentId = await getDocumentIdByTitle(form.title);

    // Eliminar la imagen del plato y el video de la receta
    await Promise.all([
      deleteFile(form.thumbnail, "image"),
      deleteFile(form.video, "video"),
    ]);

    // Eliminar el documento del plato de la base de datos
...
export const deleteFile = async (documentId, type) => {
  if (!fileId) return;

  try {
    // Eliminar el archivo del almacenamiento
    await storage.deleteFile(storageId, documentId);

    return true;
  } catch (error) {
    throw new Error(`Error al eliminar ${type}`, error);
  }
};
...
// (Corregir) Actualizar ingrediente
